...
public class MainActivity extends AppCompatActivity implements TaskAdapter.ItemClickListener {
        ...
        new ItemTouchHelper(new ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT | ItemTouchHelper.RIGHT) {
            ...
            public void onSwiped(RecyclerView.ViewHolder viewHolder, int swipeDir) {
                ...
                AppExecutors.getInstance().diskIO().execute(new Runnable() {
                    @Override
                    public void run() {
                        mDatabase.taskDao().deleteTaskEntry(mAdapter.getTasks().get(position));
                    }
                });
            }
    ...
    public void onItemClickListener(final int itemId) {
        Intent loadTaskIntent = new Intent(MainActivity.this, AddTaskActivity.class);
        loadTaskIntent.putExtra(AddTaskActivity.EXTRA_TASK_ID, itemId);
